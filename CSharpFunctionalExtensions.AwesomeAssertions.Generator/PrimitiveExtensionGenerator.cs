...
namespace CSharpFunctionalExtensions.AwesomeAssertions.Generator
{
        ...
        SourceProductionContext context)
    {
        ...
        if (typeParameters.Length > 0)
        {
            ...
            GenerateEnumExtensions(sb, className, fullTypeName, typeParameters, typeParamsDecl, methodName,
                fieldAccessor, genericPosition);
            GenerateNumericExtensions(sb, className, fullTypeName, typeParameters, typeParamsDecl, methodName,
                fieldAccessor, genericPosition);
        }
        else
        {
    ...
    private static void GenerateNumericExtensions(
        StringBuilder sb, string className, string fullTypeName, IReadOnlyList<ITypeParameterSymbol> typeParameters,
        string typeParamsDecl, string methodName, string fieldAccessor, int genericPosition)
        => sb.Append(typeParameters.Count switch
            {
                1 => $"""
                              ...
                              /// <summary>
                              /// Assertion extension for numeric values.
                              /// </summary>
                              public static NumericAssertions<T> {methodName}<T>(this {fullTypeName.Replace(className + typeParamsDecl, className + "<T>")} instance)
                                  where T : struct, IComparable<T>
                                  => new(instance.{fieldAccessor}, AssertionChain.GetOrCreate());

                              /// <summary>
                              /// Assertion extension for nullable numeric values.
                              /// </summary>
                              public static NullableNumericAssertions<T> {methodName}<T>(this {fullTypeName.Replace(className + typeParamsDecl, className + "<T?>")} instance)
                                  where T : struct, IComparable<T>
                                  => new(instance.{fieldAccessor}, AssertionChain.GetOrCreate());
                ...
                2 => (typeParameters[0].Name, typeParameters[1].Name) switch
                {
                    var (_, typeParam2) when genericPosition == 0 =>
                        $"""
                                 ...
                                 /// Assertion extension for numeric values.
                                 /// </summary>
                                 public static NumericAssertions<T> {methodName}<T, {typeParam2}>(this {fullTypeName.Replace(className + typeParamsDecl, className + $"<T, {typeParam2}>")} instance)
                                     where T : struct, IComparable<T>
                                     => new(instance.{fieldAccessor}, AssertionChain.GetOrCreate());
                                 ...
                                 /// Assertion extension for nullable numeric values.
                                 /// </summary>
                                 public static NullableNumericAssertions<T> {methodName}<T, {typeParam2}>(this {fullTypeName.Replace(className + typeParamsDecl, className + $"<T?, {typeParam2}>")} instance)
                                     where T : struct, IComparable<T>
                                     => new(instance.{fieldAccessor}, AssertionChain.GetOrCreate());
                    ...
                    var (typeParam1, _) when genericPosition == 1 =>
                        $"""
                                 ...
                                 /// Assertion extension for numeric values.
                                 /// </summary>
                                 public static NumericAssertions<E> {methodName}<{typeParam1}, E>(this {fullTypeName.Replace(className + typeParamsDecl, className + $"<{typeParam1}, E>")} instance)
                                     where E : struct, IComparable<E>
                                     => new(instance.{fieldAccessor}, AssertionChain.GetOrCreate());

                                 /// <summary>
                                 /// Assertion extension for nullable numeric values.
                                 /// </summary>
                                 public static NullableNumericAssertions<E> {methodName}<{typeParam1}, E>(this {fullTypeName.Replace(className + typeParamsDecl, className + $"<{typeParam1}, E?>")} instance)
                                     where E : struct, IComparable<E>
                                     => new(instance.{fieldAccessor}, AssertionChain.GetOrCreate());
